...
public class OverlayView extends View {
    ...
    private Paint mDimmedStrokePaintbg = new Paint(Paint.ANTI_ALIAS_FLAG);
...
//        }
        ...
        if (height > mThisHeight) {
            int width = (int) (mThisHeight * mTargetAspectRatio);
            int halfDiff = (mThisWidth - width) / 2;
            mCropViewRect.set(getPaddingLeft() + halfDiff, getPaddingTop() + (mThisHeight / 2 - width / 2),
                    getPaddingLeft() + width + halfDiff, getPaddingTop() + (mThisHeight / 2 + width / 2));
        } else {
            height = height / 2;
            int halfDiff = (mThisHeight - height) / 2;
            mCropViewRect.set(getPaddingLeft() + (mThisWidth / 2 - height / 2), getPaddingTop() + halfDiff,
                    getPaddingLeft() + +(mThisWidth / 2 + height / 2), getPaddingTop() + height + halfDiff);
        }
...
//        }
    ...
    protected void drawDimmedLayer(@NonNull Canvas canvas) {
        ...
        if (mCircleDimmedLayer) { // Draw 1px stroke to fix antialias
            ...
            /**增加一个白色圆圈框仿qq*/
            mDimmedStrokePaintbg.setColor(Color.WHITE);
            mDimmedStrokePaintbg.setStyle(Paint.Style.STROKE);
            mDimmedStrokePaintbg.setStrokeWidth(3);
            canvas.drawCircle(mCropViewRect.centerX(), mCropViewRect.centerY(), Math.min(mCropViewRect.width(), mCropViewRect.height()) / 2.f, mDimmedStrokePaintbg);
        }
    }
